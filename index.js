...
async function render(data, done) {
    ...
    const stdout = stdio && stdio.stdout ? function(data) {
        stdio.stdout.write(data);
    } : noop;

    const stderr = stdio && stdio.stderr ? function(data) {
    ...
    data.forEach(function(entry) {
        ...
        let input = entry.src || entry.input;
        let outputs = entry.dest || entry.output;
